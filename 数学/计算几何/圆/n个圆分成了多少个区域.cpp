...
void solve(){
    ...
    rep(i,1,1+n) {
        rep(j,1,1+n) if(i!=j){
            auto tmp=isCC(o[i],r[i],o[j],r[j]);
            for(auto &cur:tmp) pts[i].pb(cur),tot.pb(cur);
        }
        if(pts[i].empty()){
            P x={o[i].x-r[i],o[i].y};
            tot.pb(x);
    ...
    rep(i,1,1+n){
        E+=SZ(pts[i]);
        int x=lower_bound(all(tot),pts[i][0])-tot.begin()+1;
        for(auto &cur:pts[i]){
            int y=lower_bound(all(tot),cur)-tot.begin()+1;
            d.merge(x,y);
        }
    }
    int C=d.size();
