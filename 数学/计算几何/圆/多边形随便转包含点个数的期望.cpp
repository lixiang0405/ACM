...
最近她在玩一个无双割草类的游戏，平面上有 n 个敌人，

每一个敌人的坐标为 (xi,yi)。

可怜有一个技能是在平面上画一个 m 个点的简单多边形，

并消灭所有严格在多边形内部的敌人。

不难发现如果想要快速的消灭敌人的话，

只要画一个足够大的简单多边形就行了。但是这样的游戏性就太差了。

于是可怜打算为游戏增加一定的随机性。

可怜在平面上随便画了一个 m 个点的简单多边形 (ai,bi)。

接下来可怜打算按照 [−π,π) 上的均匀分布随机选取数字 

α（可以理解为等概率选取），并把这个简单多边形绕原点逆时针旋转 α 的角度（弧度制）。

现在可怜给你了每一个点的坐标，多边形的坐标，

你的任务是帮助可怜计算在随机旋转后她期望可以消灭多少个敌人。
*/
...
db solve(P x){
    if(x==P(0,0)){
        return contain(ps,x)==2?1:0;
    }
    db ans=0,r=x.abs();
    rep(i,0,m){
        ans+=CCT(r,ps[i],ps[(i+1)%m]);
    }
    ans/=2*PI;
    return ans;
}
